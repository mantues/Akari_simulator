...
import time
import akari_simulator.path_generator
    ...
    def move_joint_subscriber(
        ...
        for index, name in enumerate(request.joint_name):
            ...
            if name == "pan":
                ...
                path_pan = akari_simulator.path_generator.path_plotter(VMAX=vcc_max, ACC=acc, D1=self.akari_pan,  D2=pan_pos)
            elif name == "tilt":
                tilt_pos = request.val[index]
                #self.akari_tilt = request.val[index]
                path_tilt = akari_simulator.path_generator.path_plotter(VMAX=vcc_max, ACC=acc, D1=self.akari_tilt,  D2=tilt_pos)
        path_pan_fix, path_tilt_fix = akari_simulator.path_generator.merge_list(path_pan, path_tilt)
